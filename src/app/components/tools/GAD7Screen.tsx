...
export function GAD7Screen({ onDone }: GAD7ScreenProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-6 pt-14 pb-10" style={{ background: DONE_BG }}>
      ...
      >
        <span style={{ fontFamily: "Lora, serif", fontSize: 40, fontWeight: 700, color: "#ffffff" }}>{score}</span>
      </div>
      ...
      <div className="w-full max-w-sm rounded-2xl px-5 py-4 mb-5" style={{ background: "rgba(255,255,255,0.75)" }}>
        <p className="mb-3" style={{ fontFamily: "Inter, sans-serif", fontSize: 11, fontWeight: 700, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em" }}>Score Breakdown</p>
        {[{ label: "0–4", band: "Minimal", color: "#059669" }, { label: "5–9", band: "Mild", color: "#D97706" }, { label: "10–14", band: "Moderate", color: "#EA580C" }, { label: "15–21", band: "Severe", color: "#DC2626" }].map((b) => (
          <div key={b.band} className="flex items-center justify-between mb-2">
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: "#374151" }}>{b.band}</span>
            <span style={{ fontFamily: "Inter, sans-serif", fontSize: 12, color: b.color, fontWeight: 600 }}>{b.label}</span>
          </div>
        ))}
        <div className="flex items-center justify-between mt-3 pt-3" style={{ borderTop: "1px solid #F3F4F6" }}>
          ...
          <span style={{ fontFamily: "Inter, sans-serif", fontSize: 13, fontWeight: 700, color: band.color }}>{score} / 21</span>
        </div>
      </div>

      <div className="w-full max-w-sm rounded-2xl px-5 py-4 mb-8" style={{ background: "rgba(255,255,255,0.7)" }}>
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: BLUE_LIGHT }}>
